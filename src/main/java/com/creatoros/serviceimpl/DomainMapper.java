...
public class DomainMapper {
    ...
    public BrandDealDto toDealDto(BrandDeal deal) {
        return new BrandDealDto(idOf(deal.getId()), deal.getDealNumber(), deal.getBrandName(), deal.getBrandLogo(), deal.getCategory(),
                ...
                toUsageRightsDto(deal.getUsageRights()), deal.getNegotiationNotes(), deal.getPaymentTerms(), idOf(deal.getInvoiceId()),
    ...
    public InvoiceDto toInvoiceDto(Invoice invoice) {
        return new InvoiceDto(idOf(invoice.getId()), invoice.getInvoiceNumber(), invoice.getBrandName(), invoice.getBrandGstin(),
                invoice.getBrandAddress(), invoice.getCreatorName(), invoice.getCreatorGstin(), invoice.getCreatorPan(),
                toInvoiceBankDto(invoice.getBankDetails()), invoice.getIssueDate(), invoice.getDueDate(),
                invoice.getItems() == null ? List.of() : invoice.getItems().stream().map(this::toInvoiceItemDto).toList(), invoice.getSubtotal(),
                invoice.isInterstate(), invoice.getCgstAmount(), invoice.getSgstAmount(), invoice.getIgstAmount(), invoice.getTotalGst(),
                invoice.getTdsDeducted(), invoice.getTotalAmount(), invoice.getNetReceivable(), invoice.getStatus(), idOf(invoice.getDealId()),
                invoice.getPaidDate(), invoice.getReminderSentCount(), invoice.getLastReminderDate(), invoice.getExpectedSettlementDate());
    }

    private InvoiceItemDto toInvoiceItemDto(InvoiceItem item) {
        return new InvoiceItemDto(idOf(item.getId()), item.getDescription(), item.getSacCode(), item.getQuantity(), item.getUnitPrice(),
                item.getAmount());
    }
    ...
    private InvoiceBankDto toInvoiceBankDto(InvoiceBankSnapshot bank) {
        ...
        return new InvoiceBankDto(bank.getBankName(), bank.getAccountNumber(), bank.getIfscCode(), bank.getUpiId());
    }
    ...
    public NotificationDto toNotificationDto(Notification notification) {
        return new NotificationDto(idOf(notification.getId()), notification.getTitle(), notification.getMessage(), notification.getCreatedAt(),
                notification.getType(), notification.isRead(), notification.getActionUrl(), notification.getAmount());
    }
}
