...
public class NotificationDaoImpl extends HibernateDao implements NotificationDao {
    ...
    public List<Notification> findByCreatorIdOrderByCreatedAtDesc(Long creatorId) {
        return session().createSelectionQuery("from Notification n where n.creator.id = :creatorId order by n.createdAt desc", Notification.class)
                .setParameter("creatorId", creatorId).getResultList();
    }

    @Override
    public Optional<Notification> findByIdAndCreatorId(Long id, Long creatorId) {
        return session().createSelectionQuery("from Notification n where n.id = :id and n.creator.id = :creatorId", Notification.class)
